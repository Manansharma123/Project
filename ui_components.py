...
class UIComponents:
    ...
    def render_mode_selector():
        """Render enhanced mode selector with custom styling and return current mode"""
        ...
        # Display and return current mode
        ...
        st.markdown(f"#### Current Mode: {mode_names[current_mode]} {mode_descs[current_mode]}")
        return current_mode

    @staticmethod
    def validate_inputs(groq_api_key, selected_mode):
        """
        Validate the Groq API key and the selected mode.
        Returns (True, "") if valid; (False, "reason") if invalid.
        """
        if not groq_api_key or groq_api_key.strip() == "":
            return False, "Please enter your Groq API key."
        if selected_mode not in ["rag", "search", "hybrid"]:
            return False, "Please select a valid mode."
        return True, ""
